    ...
    <div class="h-full flex flex-col bg-slate-900 text-slate-200">
        ...
        <div class="flex flex-col flex-1 min-h-0 px-2 gap-4">
            ...
            <div>
                <div class="flex items-center justify-between gap-2">
                    <div class="relative flex-shrink-0 flex-1">
                        ...
                        <input :disabled="!isConnected" type="search" v-model="searchValue"
                    ...
                    <div class="relative">
                        <select v-model="sortValue" :disabled="!isConnected"
                        ...
                        <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                            <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960" width="20px"
                                fill="currentColor" class="text-slate-400">
            ...
            <div class="overflow-y-auto scrollbar flex-1 min-h-0 pr-1 focus:outline-none select-none pb-4">
                <div v-if="!charactersStore.characters" class="text-center text-slate-400">
                    Loading characters...
                </div>
                ...
                <NoInternetState v-else-if="!isConnected" @check-connection="checkConnection"></NoInternetState>
                ...
                <div v-else class="relative">
                    <Transition enter-active-class="transition-opacity duration-300 ease-out"
                        ...
                        <div v-if="spineStore.isPlayerLoading"
                            class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[9999] flex items-center justify-center">
                            <div class="flex flex-col items-center gap-3 text-white">
                                <div
                                    class="animate-spin rounded-full h-8 w-8 border-2 border-white/70 border-t-transparent">
                                </div>
                                <span class="text-sm font-medium tracking-wide">Loading character...</span>
                            </div>
                        </div>
                    </Transition>
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-3 gap-4">
                        <div v-for="character in filteredCharacters" :key="character.id"
                            ...
                            <img v-if="visibleOnce" loading="lazy" :src="getThumbnail(character)"
